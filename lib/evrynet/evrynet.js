...
exports.getWarpContract = getWarpContract;
...
function getWarpContract(address) {
...
class Evrynet {
  ...
  getNonceFromPriv(priv) {
    return new Promise((resolve, reject) => {
      ...
      var chan = this.client.GetNonce({
        evrynetAddress: account.address
      });
      chan.on('data', data => {
        resolve(data);
      });
...
class WarpContract {
  ...
  _newWarpContract(contractAddr) {
    ...
    let abi = _fs.default.readFileSync(abiPath + "/abi/warpABI.json");
  ...
  newCreditLockTx(asset, amount, priv, nonce) {
    ...
    let bnAmount = new _bignumber.default(amount).mul(StellarOne).toString();
    ...
    let tx = new _ethereumjsTx.Transaction(rawTx);
  ...
  newNativeLockTx(amount, priv, nonce) {
    ...
    let bnAmount = new _bignumber.default(amount).mul(StellarOne).toNumber();
